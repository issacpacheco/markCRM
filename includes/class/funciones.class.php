...
class funciones {
    ...
    public function cuentarray($arreglo) {
        if (is_array($arreglo)) {
    ...
    public function cuentarrayX($arreglo, $dato) {
        if (is_array($arreglo)) {
            $cuantos = count($arreglo[$dato]);
        } else {
            $cuantos = 0;
        }
        return $cuantos;
    ...
    public function redondeaTiempo($fechahora, $minutos){
        ...
        $hora = date("G:i:s",strtotime($fechahora));

        $horaactividad=date("G",strtotime($hora));
        $minutoactividad=date("i",strtotime($hora));

        if($minutoactividad % $minutos == 0){
            $ntime = str_pad($minutoactividad, 2, "0", STR_PAD_LEFT);            
            $horaactividad = str_pad($horaactividad, 2, "0", STR_PAD_LEFT);
            $horaredonda = $horaactividad.":".$minutoactividad;
        }else{
           $ntime = round($minutoactividad/$minutos, 0)*$minutos; 
            if($ntime == 60){
                $ntime = '00';
                $horaactividad++;
                ...
                if($horaactividad == 24){
                    $horaactividad = '00';
                    $fecha = $this->sumadias($fecha, 1);
                }
            }else{
                $ntime = str_pad($ntime, 2, "0", STR_PAD_LEFT);    
            }           

            $horaactividad = str_pad($horaactividad, 2, "0", STR_PAD_LEFT); 
            $horaredonda = $horaactividad.":".$ntime;           
        }     

        $fechareturn = $fecha." ".$horaredonda;
        return $fechareturn;     
    }    
    ...
    public function restahora($fechahora, $minutos){	        
        $nuevafechab = strtotime ( '-'.$minutos.' minute' , strtotime ($fechahora));
        $nuevafecha = date ('Y-m-d G:i:s', $nuevafechab);       
    ...
    public function fechaMexicana($fecha) {
        ...
        $nueva = utf8_encode($dia) . " de " . $mesnuevo . " de " . $year;
        return $nueva;
    }
    
    public function fechaCotizador($fecha) {
        $fechanueva = explode("-", $fecha);
        $year = $fechanueva[0];
        $mes = $fechanueva[1];
        $dia = $fechanueva[2];
        ...
        $mesnuevo = $this->mesAbreviado($mes);
        $nueva = utf8_encode($dia) . " - " . $mesnuevo . " - " . $year;
        return $nueva;
    }    
    ...
    public function fechaBilling($fecha){
        //YYYY-MM-DD
        ...
        $ndia = $this->diaSemanaEN($fecha);
        $mesEN = $this->nombremesEN($mes);
        
        $res = $this->mayusculas($ndia).", ".$this->mayusculas($mesEN)." ".$this->ceros($dia, 2).", ".$year;
        
        return $res;
    }
    
    public function mesAbreviadoEN($mes) {
        switch ($mes) {
            case 1: return 'Jan';
                break;
            case 2: return 'Feb';
    ...
    public function nombremesEN($mes) {
        switch ($mes) {
    ...
    public function limpianombreprospecto($cadena) {
	$no_permitidas= array ("'",",","Ã¡","Ã©","Ã­","Ã³","Ãº","Ã�","Ã‰","Ã�","Ã“","Ãš","Ã±","Ã€","Ãƒ","ÃŒ","Ã’","Ã™","Ãƒâ„¢","Ãƒ ","ÃƒÂ¨","ÃƒÂ¬","ÃƒÂ²","ÃƒÂ¹","Ã§","Ã‡","ÃƒÂ¢","Ãª","ÃƒÂ®","ÃƒÂ´","ÃƒÂ»","Ãƒâ€š","ÃƒÅ ","ÃƒÅ½","Ãƒâ€�","Ãƒâ€º","Ã¼","ÃƒÂ¶","Ãƒâ€“","ÃƒÂ¯","ÃƒÂ¤","Â«","Ã’","ÃƒÂ�","Ãƒâ€ž","Ãƒâ€¹");
    ...
    public function limpiaTelefono($cadena) {
	$no_permitidas= array ("+");
	$permitidas= array ("");
	$texto = str_replace($no_permitidas, $permitidas ,$cadena);
    ...
    public function acentosMayusculas($cadena){
    ...
    public function saltosdelinea($cadena, $caracter){
	$no_permitidas= array ($caracter);
	$permitidas= array ("\n");
	$texto = str_replace($no_permitidas, $permitidas ,$cadena);
    ...
    public function saltosdelineaGuiones($cadena, $buscar, $cambiar_Por){
	$no_permitidas= array ($buscar);
	$texto = str_replace($no_permitidas, $cambiar_Por ,$cadena);
	...
	$no_permitidas= array (" ", "'");
	$permitidas= array ("-", "");
	$texto = str_replace($no_permitidas, $permitidas ,$url);
        
	return $texto;         
    }

    public function quitarEspacios($url){
        $no_permitidas= array (" ", "'");
        $permitidas= array ("-", "");
        $texto = str_replace($no_permitidas, $permitidas ,$url);
    ...
    public function limpiaFileName($filename){
	$no_permitidas= array (" ", "'", "/", "\\");
	$permitidas= array ("-", "", "-", "-");
	$texto = str_replace($no_permitidas, $permitidas ,$filename);
    ...
    public function letraCapital($cadena){
        $min = $this->minusculas($cadena);
        $capital  = ucwords($min);
    ...
    public function primeraMayuscula($cadena){
        $min = $this->minusculas($cadena);
        $capital  = ucfirst($min);
    ...
    public function formatoMoneda($mon, $monto){
        if($mon == 'USD'){
            $mon = number_format($monto, 2, '.', ',');
            $total = "USD ".$mon;
        }else{
            $mon = number_format($monto, 2, '.', ',');
            $total = "MXN ".$mon;
    ...
    public function decimales($numero){
        if($numero >= 1){
            $num = floor(($numero*100))/100;       
        }else{
            $num = 0;
        }
    ...
    public function numOrdinalEspanol($num)
    {
        $first_word = array('eth','Primer','Segundo','Tercer','Cuarto','Quinto','Sexto','Septimo','Octavo','Noveno','Décimo','Undécimo','Duoécimo','Décimo tercer','Décimo cuarto','Décimo Quinto','Décimo sexto','Décimo septimo','Décimo Octavo','Décimo noveno','Vigésimo');
        $second_word =array('','','Veinte','Treinta','Cuarenta','Cincuenta');
    ...
    public function moneda($importe, $simbolo=''){
        if($importe > 0){
        if($simbolo == ''){
            $nuevo = number_format($importe, 2, '.', ',');
        }else{
            $nuevo = number_format($importe, 2, '.', ',');
        }}else{
            $nuevo = "0.00";
